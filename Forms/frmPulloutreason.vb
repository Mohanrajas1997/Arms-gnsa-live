...
#End Region
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Select Case ""
            Case txtPulloutReason.Text
                MsgBox("Pullout reason cannot be empty !", MsgBoxStyle.Critical, gsProjectName)
                txtPulloutReason.Focus()
                Exit Sub
        End Select
        ...
        If txtId.Text.Trim = "" Then
            ...
            Dim name As String = txtPulloutReason.Text
            Using cmd As New MySqlCommand("pr_arms_mst_tpulloutreason", gOdbcConn)
                Call ConOpenOdbc(ServerDetails)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.AddWithValue("?pr_pulloutreason_gid", 0)
                cmd.Parameters("?pr_pulloutreason_gid").Direction = ParameterDirection.Input
                cmd.Parameters.AddWithValue("?pr_pullout_reason", name)
        ...
        Else
            ...
            Using cmd As New MySqlCommand("pr_arms_mst_tpulloutreason", gOdbcConn)
                Call ConOpenOdbc(ServerDetails)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.AddWithValue("?pr_pulloutreason_gid", id)
                cmd.Parameters("?pr_pulloutreason_gid").Direction = ParameterDirection.Input
                cmd.Parameters.AddWithValue("?pr_pullout_reason", name)
                cmd.Parameters("?pr_pullout_reason").Direction = ParameterDirection.Input
    ...
    Private Sub btnFind_Click(sender As Object, e As EventArgs) Handles btnFind.Click
        ...
        Try
            SearchDialog = New frmSearch(gOdbcConn, _
                                          " SELECT pulloutreason_gid as 'Pulloutreason Id'," & _
                             " pullout_reason as 'Pullout Reason' FROM arms_mst_tpulloutreason ", _
                             " pulloutreason_gid,pullout_reason", _
                             " 1 = 1 and delete_flag = 'N'")
            SearchDialog.ShowDialog()

            If Val(txt) <> 0 Then
                Call ListAll("select * from arms_mst_tpulloutreason " _
                    & " where pulloutreason_gid = " & txt & " " _
        ...
        Try
            ...
            With ds.Tables("list_all")
                If .Rows.Count > 0 Then
                    txtId.Text = .Rows(0).Item("pulloutreason_gid").ToString
                    txtPulloutReason.Text = .Rows(0).Item("pullout_reason").ToString
                Else
                    ClearAll()
                End If
            End With
            ...
            Else
                If MsgBox("Are you sure to delete this record?", MsgBoxStyle.YesNo, gsProjectName) = MsgBoxResult.Yes Then
                    ...
                    Using cmd As New MySqlCommand("pr_arms_mst_tpulloutreason", gOdbcConn)
                        ...
                        cmd.Parameters.AddWithValue("?pr_pulloutreason_gid", id)
                        cmd.Parameters("?pr_pulloutreason_gid").Direction = ParameterDirection.Input
                        cmd.Parameters.AddWithValue("?pr_pullout_reason", 0)
                        cmd.Parameters("?pr_pullout_reason").Direction = ParameterDirection.Input
    ...
    Private Sub frmPulloutreason_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            KeyPreview = True
            Call EnableSave(False)
        Catch ex As Exception

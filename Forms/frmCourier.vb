...
#End Region
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If txtId.Text.Trim = "" Then
            ...
            Using cmd As New MySqlCommand("pr_arms_mst_tcourier", gOdbcConn)
                ...
                cmd.Parameters.AddWithValue("?pr_courier_name", name)
                cmd.Parameters("?pr_courier_name").Direction = ParameterDirection.Input
        ...
        Else
            ...
            Using cmd As New MySqlCommand("pr_arms_mst_tcourier", gOdbcConn)
                Call ConOpenOdbc(ServerDetails)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.AddWithValue("?pr_courier_gid", id)
                cmd.Parameters("?pr_courier_gid").Direction = ParameterDirection.Input
                cmd.Parameters.AddWithValue("?pr_courier_name", name)
                cmd.Parameters("?pr_courier_name").Direction = ParameterDirection.Input
    ...
    Private Sub btnFind_Click(sender As Object, e As EventArgs) Handles btnFind.Click
        ...
        Try
            SearchDialog = New frmSearch(gOdbcConn, _
                                          " SELECT courier_gid as 'Courier Id'," & _
                             " courier_name as 'Courier Name' FROM arms_mst_tcourier ", _
                             " courier_gid,courier_name", _
                             " 1 = 1 and delete_flag = 'N'")
            SearchDialog.ShowDialog()

            If Val(txt) <> 0 Then
                Call ListAll("select * from arms_mst_tcourier " _
                    & " where courier_gid = " & txt & " " _
        ...
        Try
            ...
            With ds.Tables("list_all")
                If .Rows.Count > 0 Then
                    txtId.Text = .Rows(0).Item("courier_gid").ToString
                    txtName.Text = .Rows(0).Item("courier_name").ToString
                Else
                    ClearAll()
                End If
            End With
            ...
            Else
                If MsgBox("Are you sure to delete this record?", MsgBoxStyle.YesNo, gsProjectName) = MsgBoxResult.Yes Then
                    ...
                    Using cmd As New MySqlCommand("pr_arms_mst_tcourier", gOdbcConn)
                        ...
                        cmd.Parameters.AddWithValue("?pr_courier_gid", id)
                        cmd.Parameters("?pr_courier_gid").Direction = ParameterDirection.Input
                        cmd.Parameters.AddWithValue("?pr_courier_name", 0)
                        cmd.Parameters("?pr_courier_name").Direction = ParameterDirection.Input
    ...
    Private Sub frmCourier_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            KeyPreview = True
            Call EnableSave(False)
        Catch ex As Exception

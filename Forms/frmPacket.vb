...
#End Region
    ...
    Sub ImportPacketFile()
        ...
        Dim lsRcvDate As String = ""
        Dim lslotno As String = ""
        Dim lsPackNo As String = ""
        Dim lsPackslNo As String = ""
        Dim lsCvrNo As String = ""
        Dim lsCntNo As String = ""
        Dim lsChqType As String = ""
        Dim lsTotChq As String = ""
        Dim lsEMIfrq As String = ""
        ...
        lsFldName(5) = "Packet No"
        lsFldName(6) = "Packet Serial No"
        lsFldName(7) = "Cover No"
        lsFldName(8) = "Contract No"
        lsFldName(9) = "Cheque Type"
        lsFldName(10) = "Total Cheques"
        lsFldName(11) = "Emi Frequency"
        lsFldName(12) = "Remark"
        ...
        With fExcelDatatable
            ...
            For i = 0 To .Rows.Count - 1
                ...
                lsEMIfrq = Mid(QuoteFilter(.Rows(i).Item("Emi Frequency").ToString), 1, 16).ToUpper
                If lsEMIfrq = "M" Or lsEMIfrq = "H" Or lsEMIfrq = "Q" Or lsEMIfrq = "Y" Then
                    lsEMIfrq = lsEMIfrq
                Else
                    MsgBox("Emi Frequency Should be M,H,Q,Y. line no:" & (i + 1).ToString, MsgBoxStyle.Critical, gsProjectName)
                    Exit Sub
                End If
            Next
        End With
        ...
        fsSql &= " and file_type = " & gnFilePacketUpload & " "
        ...
        cmd.Parameters.AddWithValue("?pr_file_type", gnFilePacketUpload)
        ...
        With fExcelDatatable
            ...
            While i <= .Rows.Count - 1
                With .Rows(i)
                    ...
                    lsRcvDate = QuoteFilter(.Item("Received Date").ToString)
                    If IsDate(lsRcvDate) Then
                        lsRcvDate = Format(CDate(lsRcvDate), "yyyy-MM-dd")
                    Else
                        lsRcvDate = ""
                    End If


                    lsPackNo = Mid(QuoteFilter(.Item("Packet No").ToString), 1, 255)
                    lsPackslNo = Mid(QuoteFilter(.Item("Packet Serial No").ToString), 1, 255)
                    ...
                    lsTotChq = Mid(QuoteFilter(.Item("Total Cheques").ToString), 1, 32)
                    lsEMIfrq = Mid(QuoteFilter(.Item("Emi Frequency").ToString), 1, 1)
                    lsRemark = Mid(QuoteFilter(.Item("Remark").ToString), 1, 255)
                    lslotno = Mid(QuoteFilter(.Item("Lot No").ToString), 1, 32)

                    Call ConOpenOdbc(ServerDetails)
                    cmd = New MySqlCommand("pr_arms_trn_tpacket", gOdbcConn)
                    cmd.CommandType = CommandType.StoredProcedure
                    cmd.Parameters.AddWithValue("?pr_entity_gid", cboEntity.SelectedValue)
                    cmd.Parameters.AddWithValue("?pr_file_gid", lnFileId)
                    cmd.Parameters.AddWithValue("?pr_lot_no", lslotno)
                    cmd.Parameters.AddWithValue("?pr_receive_date", CDate(lsRcvDate))
                    cmd.Parameters.AddWithValue("?pr_packet_no", lsPackNo)
                    cmd.Parameters.AddWithValue("?pr_packet_slno", lsPackslNo)
                    cmd.Parameters.AddWithValue("?pr_packet_status", GCINWARDENTRY)
                    ...
                    cmd.Parameters.AddWithValue("?pr_total_chqs", lsTotChq)
                    cmd.Parameters.AddWithValue("?pr_emi_freq", lsEMIfrq)
                    cmd.Parameters.AddWithValue("?pr_remarks", lsRemark)
                    cmd.Parameters.AddWithValue("?pr_action", "INSERT")
    ...
    Private Sub frmPacket_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try

...
Module objExcel
    ...
    Public Sub ExportDatasetToXML(ByVal dt As DataTable, ByVal FileName As String, Optional ByVal SheetName As String = "Report", Optional ByVal StoreasNum As String = "")
        Dim TotCol As Integer, Col As Integer, Row As Integer
        Dim objStreamReader As StreamReader
        Dim lsTextContent As String
        Dim NumericCols() As String
        ...
        Try
            ...
            PrintLine(1, "<Row>")
            For Col = 1 To TotCol
                PrintLine(1, "<Cell ss:StyleID=""s1""><Data ss:Type=""String"">" & dt.Columns(Col - 1).Caption.ToString & "</Data></Cell>")
            Next
            PrintLine(1, "</Row>")
            ...
            For Row = 0 To dt.Rows.Count - 1
                ...
                For Col = 1 To TotCol
                    If IsNumericFldCol(Col, NumericCols) = False Then
                        PrintLine(1, "<Cell ss:StyleID=""s2""><Data ss:Type=""String"" x:Ticked=""1"">" & dt.Rows(Row).Item(Col - 1).ToString & "</Data></Cell>")
                    Else
                        PrintLine(1, "<Cell ss:StyleID=""s2""><Data ss:Type=""Number"">" & dt.Rows(Row).Item(Col - 1).ToString & "</Data></Cell>")
                    End If
                Next
                PrintLine(1, "</Row>")
            Next
    ...
    Public Sub PrintDataTable(Sql As String, FileName As String, Optional ShowFlag As Boolean = True)
        Dim i As Long
        Dim j As Integer
        ...
        Try
            Call gpDataSet(Sql, "data", gOdbcConn, ds)

            If File.Exists(FileName) Then File.Delete(FileName)

            Call FileOpen(1, FileName, OpenMode.Output, OpenAccess.Write)

            With ds.Tables("data")
                lsTxt = ""

                For j = 0 To .Columns.Count - 1
                    lsTxt &= .Columns(j).ColumnName & Chr(9)
                Next j

                Call PrintLine(1, lsTxt)

                For i = 0 To .Rows.Count - 1
                    lsTxt = ""

                    For j = 0 To .Columns.Count - 1
                        lsTxt &= .Rows(i).Item(j).ToString() & Chr(9)
                    Next j
